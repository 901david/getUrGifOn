...
function searchForImages () {
	$(".clickSearch").on("click", function () {
		...
		$.ajax({
			...
			for (var i = 0; i < 10; i++){
				$(".imageArea").append("<div class='col-lg-3 img-responsive clickOn'><span>Rating: " + response.data[i].rating.toUpperCase() + "</span><br/><img class='grabMe' data-switch='" + response.data[i].images.fixed_width.url + "' data-switch-back='" + response.data[i].images.fixed_width_still.url + "' src='" + response.data[i].images.fixed_width_still.url + "'>");	
			}
...
$(document).ready(function(){
	...
	$.ajax({
		...
		for (var i = 0; i < 9; i++){
			...
			$(".imageArea").append("<div class='col-lg-3'><span>Rating: " + response.data[i].rating.toUpperCase() + "</span><br/><img class='img-responsive' src='" + response.data[i].images.fixed_width.url + "'>");
		};
	});
	searchForImages();
